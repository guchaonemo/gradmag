...
void gradMag( float *I, float *M, float *O, int h, int w, int d, bool full ) {
  ...
  for( x=0; x<w; x++ ) {
    ...
    for(c=0; c<d; c++) {
	  ...
	  for (y = 0; y < h4 / 4; y++)
	  {
		  M2[tid+0]   = Gx[tid + 0] * Gx[tid + 0] + Gy[tid + 0] * Gy[tid + 0];
		  M2[tid + 1] = Gx[tid + 1] * Gx[tid + 1] + Gy[tid + 1] * Gy[tid + 1];
		  M2[tid + 2] = Gx[tid + 2] * Gx[tid + 2] + Gy[tid + 2] * Gy[tid + 2];
		  M2[tid + 3] = Gx[tid + 3] * Gx[tid + 3] + Gy[tid + 3] * Gy[tid + 3];
		  if (c == 0)
		  {
			  y1++;
			  tid += 4;
	...
	for (y = 0; y < h4 / 4; y++)
	{
		tmp_m[0] = min(1.0f / sqrt(M2[tid + 0]), 1e10f);
		tmp_m[1] = min(1.0f / sqrt(M2[tid + 1]), 1e10f);
		tmp_m[2] = min(1.0f / sqrt(M2[tid + 2]), 1e10f);
		tmp_m[3] = min(1.0f / sqrt(M2[tid + 3]), 1e10f);
		M2[tid + 0] = 1.0f / tmp_m[0];
		M2[tid + 1] = 1.0f / tmp_m[1];
		M2[tid + 2] = 1.0f / tmp_m[2];
		M2[tid + 3] = 1.0f / tmp_m[3];
		if (O)
		{
			Gx[tid + 0] = acMult * tmp_m[0] * Gx[tid + 0];
			Gx[tid + 1] = acMult * tmp_m[1] * Gx[tid + 1];
			Gx[tid + 2] = acMult * tmp_m[2] * Gx[tid + 2];
			Gx[tid + 3] = acMult * tmp_m[3] * Gx[tid + 3];
		}
		if (O)
		{
			for (int k = 0; k < 4; k++)
    ...
    if( O!=0 && full ) {
      for(y=0 ; y<h; y++ ) O[y+x*h]+=(Gy[y]<0)*PI;
    }
  }
      ...
      if( softBin<0 ) 
		  for( ; ; y++ ) 
		  {
			  ...
			  { 
				  ...
				  H0[O0[y] + 0+hb] += ms[2] * M0[y];
				  H0[O0[y] + 1+hb] += ms[3] * M0[y];
			  }
      } else 
			  for( ; ; y++ ) 
			  {
				  ...
				  if(hasRt)
				  {
					  ...
					  H0[O1[y] + 0 + hb] += ms[2] * M1[y];
					  H0[O1[y] + 1 + hb] += ms[3] * M1[y];
...
float* fhog_main(float* I, int height, int width, int channel, int binSize, int nOrients, float clip, bool crop)
{
	float M_ary[128 * 128];
	float *M = M_ary;
	float O_ary[128 * 128];
	float *O = O_ary;
	gradMag(I, M, O, 128, 128, 1, true);

	float H_ary[32 * 32 * 31];
	float *H = H_ary;
	memset(H, 0.0f, 32 * 32 * 31 * sizeof(float));
	fhog(M, O, H, 128, 128, 4, 9, -1, clip);
